...
### アンカーは音源を開くたびに0秒へ戻す（仕様）
`_load()` はファイルを読み込むたびに **アンカーと再生位置を必ず 0 に戻す**。
履歴（Recent）から同じ音源を開き直した場合も同じ。

前回の練習で最後の一文にアンカーを置いて終えていても、次に開くときは
先頭から練習することが多いため、持ち越さない方が使いやすい。

したがって以下は**実装しない**。設定で切り替えられるようにもしない。
- ファイル別のアンカー保存 / アンカー位置の復元
- 最終再生位置の復元

永続化するのは「最近開いたファイルのパス」だけ（`lib/recent_files.dart`）。
この動作は `test/` で保証する。

## 実装済みの機能

（版を書くと更新漏れで嘘になるので書かない。実際の挙動は
`lib/shortcuts.dart` の `kShortcuts` と `test/` が正とする）
...
### アクセシビリティ（現状と既知の制限）
**対応済み**: `MarkerSeekBar` は1枚の `CustomPaint` だが、意味としては再生位置と
アンカーという独立した2つの値を持つ。読み上げで別物として伝わるよう、
それぞれに `Semantics`（`Playback position` / `Anchor`）を重ねてある。
描画には関与しないので大きさも当たり判定も変わらない。

**既知の制限**: `PlayerPage` は `Focus(descendantsAreFocusable: false)` で
子にフォーカスを渡していないため、**Tab キーによる通常のフォーカス移動ができない**。
これを外すと**矢印キーがフォーカス移動に奪われてシークできなくなる**
（実際に起きた不具合）。Flutter 標準の `Shortcuts`/`Actions` に載せ替えれば
グローバルショートカットとフォーカス移動を両立できるが、キー処理は繰り返し
壊れてきた箇所なので、中核操作を壊すリスクを取らず現状を維持している。
**「アクセシビリティ対応済み」ではなく「読み上げは改善、Tab 移動は未対応」**が正確。
