...
96×96 のグリッド（Material Symbols の 24px グリッド ×4）に、
ストローク幅 8・丸終端の均一な線で錨を描き、リングは塗りの円から再生三角を抜く。
Material Symbols Rounded の weight 400 相当。
...
### 配色
シード色はアイコンのプレートと同じ淡い水色。M3 が色相を保ったままトーンを割り当てるので、
アイコンと画面の色が地続きになる。

| 用途 | 色 |
|------|-----|
| シード（`ColorScheme.fromSeed`） | `#A3D8E1`（HidéToys の淡い水色） |
| 錨本体（アイコン） | `#13253F` インクネイビー |
